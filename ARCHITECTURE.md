...
Ce projet est un **agrégateur / vitrine de mode japonaise d'occasion** : il scrape des annonces de **15 marques ciblées** (lignes CdG — Homme/Plus/Deux/SHIRT —, Junya Watanabe, Undercover, Number (Nine), LGB, Tornado Mart, Pleats Please, Yohji Yamamoto, Kapital, Mastermind, Miu Miu, Ann Demeulemeester) depuis **Mercari Japan**, puis les nettoie, les traduit en anglais, les classe par marque et les stocke dans une base **SQLite**. Un petit site **Flask** affiche ensuite ce catalogue sous forme de grille filtrable (marque / taille / état / prix), avec fiche détaillée et redirection vers Mercari ou le proxy d'achat Buyee. Le tout se rafraîchit **automatiquement toutes les 6 heures** via GitHub Actions, qui recommit le catalogue et déclenche un redéploiement sur Render.
...
| 2 | [parse.py](parse.py) | **Normaliseur.** Filtre : garde uniquement les IDs Mercari réguliers (`m` + 11 chiffres, pas les « Mercari Shops »), whitelist « clothing only » (drop chaussures / sacs). Mappe les 6 labels de condition Mercari → échelle normalisée, parse une taille depuis le texte libre, **normalise le type de vêtement** (`category_norm` : T-shirt/Shirt/Jacket/Pants… — catégorie Mercari raffinée par les mots du titre quand elle est trop grossière), convertit JPY→EUR. | `listings_raw.json` → `data/listings_normalized.json` |
...
| 6 | [app.py](app.py) | **Serveur web Flask.** 3 routes : `/` (grille filtrable — marque, **catégorie de vêtement**, **tailles groupées** haut/bas/numérique, état, prix, tri ; compteur « N brands · M items »), `/item/<id>` (fiche + galerie + **date de publication**), `/go/<id>` (log le clic → SQLite + **Neon persistant** → 302 Mercari/Buyee). **Events Plausible custom** (`Item View` / `Item Detail` / `Outbound Click` avec props brand/category/dest) sans navigation supplémentaire. `DISPLAY` dérivé de brands.yaml. | `listings.db` → HTML (templates) |
